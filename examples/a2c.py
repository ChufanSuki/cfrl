...
if args.prod_mode:
    ...
    wandb.init(project=args.wandb_project_name, entity=args.wandb_entity,
               tensorboard=True, config=vars(args), name=experiment_name, monitor_gym=True)
...
input_shape, preprocess_obs_fn = preprocess_obs_space(
    env.observation_space, device)
...
pg = Policy().to(device)
vf = Value().to(device)
optimizer = optim.Adam(list(pg.parameters()) +
                       list(vf.parameters()), lr=args.learning_rate)
loss_fn = nn.MSELoss()
...
while global_step < args.total_timesteps:
    ...
    for step in range(args.episode_length):
        ...
        if isinstance(env.action_space, Discrete):
            ...
            logits_categories = torch.split(
                logits, env.action_space.nvec.tolist(), dim=1)
            action = []
            probs_categories = []
            ...
            for i in range(len(logits_categories)):
                probs_categories.append(
                    Categorical(logits=logits_categories[i]))
    ...
    optimizer.zero_grad()
    loss.backward()
    nn.utils.clip_grad_norm_(list(pg.parameters()) +
                             list(vf.parameters()), args.max_grad_norm)
    optimizer.step()
    ...
    writer.add_scalar("losses/entropy",
                      entropys[:step].mean().item(), global_step)
